...
- [Step 5 - Tagged release of Common-PowerShell](#step-5---tagged-release-of-Common-PowerShell)
...
**Changes:**
- New file
  `Common-PowerShell/.github/actions/lint-workflows/version.txt`
...
- New fixture workflows under
  `Common-PowerShell/Tests/Unit/lint-workflows/fixtures/`:
...
**Changes:**
- `Common-PowerShell/.github/actions/lint-workflows/action.yml`
...
- `Common-PowerShell/.github/actions/lint-workflows/Invoke-WorkflowLint.ps1`
  ...
  - Runs `actionlint` against
    ...
    exit code.
- Tests under
  `Common-PowerShell/Tests/Unit/lint-workflows/Invoke-WorkflowLint.Tests.ps1`:
...
- `Common-PowerShell/README.md`: document the new action under
...
- `Common-PowerShell/README.md`: extend the action's
...
## Step 5 - Tagged release of Common-PowerShell
...
- Annotated tag (next minor; if the current is `v0.x.y`, this is
  ...
  user-facing).
- Push tag to remote.
- `Common-PowerShell/README.md`: list the new tag in the
...
- Add a new step to `ci-powershell.yml` immediately after the
  ...
  - Calls the local script
    ...
    consumer's workflows, not Common-PowerShell's).
- `Common-PowerShell/README.md`: document the lint preflight in
...
```mermaid
flowchart LR
  CO[actions/checkout] --> CI[checkout Common-PowerShell into .ci-common]
...
- `Common-PowerShell/README.md`: extend each workflow's section
