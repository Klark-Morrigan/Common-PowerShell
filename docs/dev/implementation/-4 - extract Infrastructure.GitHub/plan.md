...
- [Step 5 - Remove GitHub functions from Common.PowerShell](#step-5---remove-github-functions-from-infrastructurecommon)
...
conventions as `Common.PowerShell`. CI workflows delegate to Common's
...
`Common.PowerShell/Public/...` to `Infrastructure.GitHub/Public/...` but
...
**Common.PowerShell is not changed in this step.** Both modules export the
...
graph LR
  subgraph PSGallery
    Common[Common.PowerShell - functions still present]
...
after the existing `Common.PowerShell` install. No other changes - all
...
Add `Invoke-ModuleInstall -ModuleName 'Infrastructure.GitHub' -MinimumVersion '0.1.0'`
after the existing `Common.PowerShell` install block.

### `deregister-runners.ps1`
Same addition if it installs Common.PowerShell (check and mirror).
...
## Step 5 - Remove GitHub functions from Common.PowerShell
...
graph LR
  subgraph PSGallery
    Common[Common.PowerShell v2.4.0 - GitHub functions removed]
    GitHub[Infrastructure.GitHub v0.1.0]
  end
  subgraph Consumers
    E2E[Infrastructure-E2E]
