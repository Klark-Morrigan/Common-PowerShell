# Problem: Common.PowerShell contains GitHub-specific functions
...
`Common.PowerShell` was created as a home for shared PowerShell utilities.
...
`Common.PowerShell` retains only genuinely generic utilities. Neither
...
```
PSGallery
  Common.PowerShell   (SSH, file server, generic utilities)
